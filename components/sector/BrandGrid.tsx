...
export function BrandGrid({ brands, locale }: { brands: Marka[]; locale: Locale }) {
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <p className="text-sm text-ink/70">
          {aranan ? t.found(list.length) : t.total(brands.length)}
        </p>
      ...
      {list.length === 0 ? (
        <p className="mt-10 text-center text-sm text-muted">{t.notFound}</p>
      ) : (
        <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          {list.map((b) => (
            ...
            >
              ...
              <p className="mt-0.5 text-xs text-muted">{t.products(b.count)}</p>
            </div>
          ))}
        </div>
      )}
